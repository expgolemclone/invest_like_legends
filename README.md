...
- 株価、ネットキャッシュ比率、実績 PER、今期予想 PER、来期予想 PER、PEG 5、優先株有無、自己資本比率、平均 FCF 利回り、CROIC を同じテーブルで確認する
...
PER と PEG 5 は `formula_screening` が `stock_db` の財務データから計算した値を使い、実績 PER・今期予想 PER・来期予想 PER・実績 PER / 過去 5 期純利益 CAGR[%] を分けて表示します。
...
起動時には `stock_db` の Stooq 更新APIを呼び、株価が JPX 営業日ベースで古い場合だけ更新してからデータを生成します。成功した更新チェックは記録されるため、Stooq 側のデータがまだ進んでいない場合も起動のたびに再実行されることはありません。
